<script setup>
// D3 calcula el layout solo en desktop; en móvil renderiza lista con barra doble.
...
// < 640px → lista móvil; ≥ 640px → treemap
const esMovil = computed(() => ancho.value > 0 && ancho.value < 640)

// Items ordenados para la lista móvil (todos, sin agrupar)
const itemsOrdenados = computed(() =>
  [...props.items].sort((a, b) => Number(b.apropiado) - Number(a.apropiado))
)

function pctEjecucion(item) {
  const ap = Number(item.apropiado)
  const pa = Number(item.pagado)
  return ap ? Math.min(100, Math.round(pa / ap * 1000) / 10) : 0
}

function colorEjecucion(pct) {
  if (pct >= 70) return '#2E8B57'
  if (pct >= 40) return '#C9843A'
  return '#C0392B'
}
...
// ── Agrupa ítems <1% en "Otros" (solo se usa en el treemap) ─────────────────
...
const rects = computed(() => {
  ...
  treemap().tile(treemapSquarify).size([ancho.value, alto.value]).paddingInner(3)(raiz)
...
<template>
  <div ref="contenedor" class="relative w-full select-none" @keydown="tecla">
    ...
    <!-- ══ MÓVIL: lista con barra asignado / pagado ══════════════════════ -->
    <div
      v-if="esMovil"
      class="rounded-xl overflow-hidden border bg-white"
      style="border-color: rgba(107,118,134,0.13);"
    >
      <button
        v-for="item in itemsOrdenados"
        :key="item.nombre"
        class="w-full flex flex-col gap-2 px-4 py-3.5 text-left border-b last:border-b-0 active:bg-papel transition-colors"
        style="border-color: rgba(107,118,134,0.08);"
        :disabled="esUltimoNivel"
        @click="clickNodo(item)"
      >
        <!-- Fila 1: punto de color · nombre · monto · flecha -->
        <div class="flex items-center gap-2.5">
          <span
            class="flex-shrink-0 w-3 h-3 rounded-sm"
            :style="{ background: infoColor(item).color }"
          ></span>
          <span class="flex-1 text-sm font-semibold text-tinta min-w-0 leading-snug">
            {{ item.nombre }}
          </span>
          <span class="text-xs tabular text-neutro shrink-0 ml-1">
    ...
    <template v-else>
      ...
      >
        ...
        >
            ...
            <span
              ...
              :style="{
                ...
                background: infoColor(r.item).textColor === '#FFFFFF'
                  ? 'rgba(255,255,255,0.8)' : 'rgba(21,41,74,0.55)',
              }"
            ></span>
          ...
          <div v-if="r.pw > 88 && r.ph > 82" style="font-size: 11px; opacity: 0.82;">
      ...
      >
        ...
        >
          <div class="flex items-center justify-between px-5 py-4 border-b"
            ...
            <div>
              <h3 class="font-display font-bold text-tinta text-base">Otros sectores</h3>
              <p class="text-xs text-neutro mt-0.5">{{ itemsOtros.length }} sectores · toca uno para ver el detalle</p>
            </div>
            <button
              class="w-9 h-9 flex items-center justify-center rounded-full text-neutro text-xl font-light"
